...
struct VertexInputs
{
    ...
    float3 i_normal [[attribute(1)]];
    float3 i_tangent [[attribute(2)]];
};
struct VertexData
{
    float4 pos [[position]];
...
,     i_tangent(i_tangent)
    ...
    VertexData VertexMain()
    {
        ...
        vd.normalWorld = normalize(mx_matrix_mul(u_worldInverseTransposeMatrix, float4(i_normal, 0.0)).xyz);
        vd.tangentWorld = normalize(mx_matrix_mul(u_worldMatrix, float4(i_tangent, 0.0)).xyz);
...
VertexInputs i_vs [[ stage_in ]], constant PrivateUniforms& u_prv[[ buffer(3) ]])
{
    ...
    VertexData outVertex = ctx.VertexMain();
    outVertex.pos.y = -outVertex.pos.y;
    return outVertex;
}
